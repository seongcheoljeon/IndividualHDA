...
class RecordModel(QtCore.QAbstractItemModel):
    ...
    def insert_record_data(self, data: Any = None) -> None:
        self.__insert_item_to_record_data(data=self.__data, insert_data=data)

    def __insert_item_to_record_data(
            ...
            for key, val in insert_data.items():
                ...
                if get_data is None:
                    ...
                    if isinstance(get_data, list):
                        ...
                        self.__insert_item_to_record_data(
    ...
    def remove_invalid_hull_record_item_model(self) -> None:
        # 데이터를 삭제하고 빈 껍데기만 남은 데이터가 있다면 삭제하는 함수
        invalid_data_list = self.__find_invalid_hull_record_item_model(
            index=self.index(0, 0, QtCore.QModelIndex())
        )
    ...
    def __get_record_data_from_key_data(
    ...
    def __find_invalid_hull_record_item_model(
        ...
        for row in range(0, self.rowCount(index)):
            ...
            if is_invalid_data:
                ...
                continue
            find_invalid_lst += self.__find_invalid_hull_record_item_model(
                index=child_index
            )
    ...
    def __is_exist_invalid_record_data(self, index: QtCore.QModelIndex = None) -> bool:
        is_found = True
    ...
    def __remove_item_from_record_data(
        ...
        if isinstance(data, dict) and isinstance(remove_key_data, dict):
            ...
            if isinstance(data, list):
                ...
                # record data 이름만 가져옴
                record_name_lst = [x[2] for x in data]
    ...
    def remove_invalid_record_data(self) -> None:
        ...
        if role == QtCore.Qt.ItemDataRole.TextAlignmentRole:
            ...
            if rdata is None:
                ...
                disp_lst = (
                    ...
                    node.ctime,
                    node.mtime,
                    rdata.get(public.Key.Record.houdini_version),
                    rdata.get(public.Key.Record.houdini_license),
                    rdata.get(public.Key.Record.operating_system),
                    rdata.get(public.Key.Record.sf),
                    rdata.get(public.Key.Record.ef),
                    rdata.get(public.Key.Record.fps),
                )
