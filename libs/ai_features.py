...
def structure_from_asset(asset: AssetData) -> dict[str, Any]:
    """What the library already knows about the node; safe to send as-is."""
    return {
        "type": asset.get("node_type_name", ""),
        "category": asset.get("hda_cate", ""),
        "definition": asset.get("node_def_desc", ""),
        "is_network": bool(asset.get("is_network")),
        "is_sub_network": bool(asset.get("is_sub_network")),
        "inputs": len(asset.get("node_input_connections") or ()),
        "outputs": len(asset.get("node_output_connections") or ()),
        "children": asset.get("children") or {},  # only known at registration time
...
def describe_asset(
    ...
    *,
    ...
    prompt = build_describe_prompt(
        str(asset.get("hda_name", "")),
        structure or structure_from_asset(asset),
        existing_note=str(asset.get("hda_note") or ""),
        existing_tags=list(asset.get("hda_tags") or []),
