...
class CatalogOperations(DatabaseSession):
    def insert_users(
        ...
        VALUES (:user_id, :email, (SELECT DATETIME('now', 'localtime')))
        """
        try:
            dat: dict[str, Any] = {"user_id": user_id, "email": email}
            cursor = self._cursor.execute(query, dat)
            self._commit()
            return cursor.rowcount
        except Exception as err:
            self._rollback()
            logging.error("*** users (insert) ***")
            logging.error(err)
            return None
    ...
    def insert_hda_category(
        ...
        INSERT INTO hda_category (category, user_id)
            ...
            WHERE NOT EXISTS (SELECT * FROM hda_category WHERE category = :category AND user_id = :user_id)
        """
        try:
            dat: dict[str, Any] = {"category": category, "user_id": user_id}
            cursor = self._cursor.execute(query, dat)
            self._commit()
            return cursor.rowcount
        except Exception as err:
            self._rollback()
            logging.error("*** hda_category (insert) ***")
            logging.error(err)
            return None
    ...
    def insert_hda_key(
        ...
        query = "INSERT INTO hda_key (name, category, user_id) VALUES (:name, :category, :user_id)"
        try:
            dat: dict[str, Any] = {
                "name": name,
                "category": category,
            ...
            self._rollback()
            logging.error("*** hda_key (insert) ***")
            logging.error(err)
            return None

    def delete_hda_key_with_id(self, hda_key_id: int | None = None) -> int | None:
        query = "DELETE FROM hda_key WHERE id = :hda_key_id"
