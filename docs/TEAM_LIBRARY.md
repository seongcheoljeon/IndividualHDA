...
현재 버전 추적 기능은 SQLite 6 / PostgreSQL 3을 사용하며 HTTP API 2는 유지합니다.
...
`init-db`는 신규 설치에 스키마 버전 3을 설치합니다. 서버 시작은 스키마를 확인할 뿐
...
## 개인 스키마 6 / 팀 스키마 3 업그레이드

개인 DB는 스키마 6, 팀 DB는 스키마 3 / HTTP API v2를 사용합니다. 새 추적 기능은
팀 서버를 업그레이드한 뒤 사용할 수 있습니다. API v1 요청은 426 응답으로 차단하며
클라이언트는 연결 시 API 버전을 확인합니다. API v2 서버는 계속 연결할 수 있으며
`version_tracking` 기능이 없는 서버에서는 새 추적 탭을 숨깁니다. 개인 DB는 처음 열 때 백업 후 단계별로 업그레이드합니다.
...
- `Version details`의 Tracking에서 수동 호환성 검사 결과·정정 이력, 참조 에셋,
  씬 사용 위치를 확인합니다. 검사 환경·범위는 사용자가 입력하며 자동 Houdini 실행은 없습니다.
- Team 씬 경로와 노드 위치는 프로젝트 구성원에게 공유됩니다. viewer도 자신의 관측을
  기록할 수 있지만 검사 보고서·의존성 편집은 editor/owner만 할 수 있습니다.
- 오프라인 관측은 클라이언트 SQLite 큐에 남고 동일 요청으로 재전송합니다. 상태 표시줄의
  미전송 안내가 남으면 `Library Tools → Retry scene reporting`을 사용합니다.
- 참조 중인 버전을 Trash로 옮기거나 영구 삭제할 때 영향을 안내합니다. 사용자가 계속
  진행할 수 있으며, 남은 의존성은 UUID와 원래 입력한 텍스트를 보존합니다.
- `Pending registrations`는 완료된 캡처·업로드를 재시도합니다. 캡처 도중 중단된 항목은
  Houdini에서 다시 캡처해야 합니다. 서버로 보낸 요청은 Retry로 결과를 확인한 뒤 정리합니다.
- 새 백업에는 추적 테이블이 포함됩니다. 스키마 2 백업도 빈 DB로 복원한 뒤 명시적으로
  업그레이드할 수 있습니다. 클라이언트 캡처/재전송 큐는 서버 백업에 포함되지 않습니다.
