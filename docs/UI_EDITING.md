...
| 현재 HIP의 iHDA 노드 스캔(Find 탭) | `widgets/panel/inside_page.py` `InsidePageController`; 스캔 `libs/houdini/assets.py scan_ihda_nodes()` → `libs/scene_scan.py` → `model/ihda_inside_model.py` | 페이지 로직(스캔·콤보·카운트·빈 상태)은 컨트롤러에만; 갱신 시점은 `inside_page().mark_stale()` |
| 검색·에셋 목록·확대·화면 전환 | `widgets/asset_browser/view.py` | `_build_toolbar()`, `_build_search_row()` |
| 항목 모양 — 그리드 카드·표 행·카테고리 배지 | `widgets/item_delegates.py` | `CardDelegate`, `RowDelegate`, `CountBadgeDelegate` (모델 `data()`는 데이터만, 그리기는 여기) |
...
| 도움말(Help) | `widgets/help_dialog.py` | `GESTURES`·`SHORTCUTS`·`TABS` 표에 줄 추가 (`SHORTCUTS`는 `widgets/panel/shortcuts.py`가 바인딩한 키와 일치해야 함) |
| 정보(About) | `widgets/about_dialog.py` | `environment_facts()`(표시·복사 항목), `AboutDialog.__init__`(링크 버튼); 라이선스 본문은 `LICENSE`·`THIRD_PARTY_LICENSES.txt`에서 읽음 |
