...
| Read something from the current HIP file (nodes, comments, scene state) | Add a method to `HostScenePort` (`libs/host_ports.py`), implement it in `libs/houdini/*` returning plain dataclasses (see `libs/scene_scan.py`), and hand those to the model; tests then drive the page with a fake port (`test_qt.py` `FakeScene`) | `houdini_api.HoudiniAPI` calls inside a model or a Qt slot |
| Tell the user something happened | `self.bindings.notifications.notify(message, action="Undo", on_action=…, level=…)` (`NotificationsPort`; the presentation feature implements it) — a toast (`widgets/toast.py`), the status bar and the log in one call; keep `QMessageBox` for questions and irreversible steps (purge, dependents) | An information `QMessageBox`, or a log line the user never sees |
